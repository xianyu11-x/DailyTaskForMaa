...
def main():
    # 使用argparse处理命令行参数
    parser = argparse.ArgumentParser(description='发送MAA任务请求')
    parser.add_argument('-u', '--user', required=True, help='用户名')
    parser.add_argument('-d', '--device', required=True, help='设备名称')
    parser.add_argument('-t', '--task', required=True, help='任务JSON文件名')
    ...
    # 构建完整URL
    server_ip = args.url
    url = f"http://{server_ip}:8080/maa/quickTask"
    
    # 构建JSON文件路径（当前脚本目录下的指定JSON文件）
    current_dir = Path(__file__).parent
    json_path = current_dir / f"{task_file}"
    ...
    try:
        ...
        if isinstance(task_data, dict):
            if "user" in task_data:
                task_data["user"] = user
            if "device" in task_data:
                task_data["device"] = device
